class Order < ActiveRecord::Base
  ...
  def self.largest_value(last_x_days = nil)
    if last_x_days
      ...
      Order.select("SUM(purchases.quantity * products.price) AS value").joins("JOIN purchases ON orders.id = purchases.order_id JOIN products ON products.id = purchases.product_id").where(:checked_out => true).order("value DESC").group("orders.id").first.value
    end
  end
